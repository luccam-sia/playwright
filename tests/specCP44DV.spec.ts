...
test.describe('Filtro de inferencias por Dispositivo', () => {
    ...
    test('El filtro de inferencias por dispositivo funciona', async ({ page }) => {
        ...
        await inferencePage.combineFilters('GT RK3588 8 l', 'LOC QA 2', 'GRUPO PRUEBA', 0, 21);
        ...
        inferences.forEach((inference: any, index: number) => {
            ...
            expect(inference.boxId, `Error en la inferencia #${index + 1} (ID: ${inference.id})`).toBe(18);
            expect(inference.locationId, `Error en la inferencia #${index + 1} (ID: ${inference.id})`).toBe(3);
            expect(inference.groupId, `Error en la inferencia #${index + 1} (ID: ${inference.id})`).toBe(2);
