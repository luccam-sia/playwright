...
test.describe('Filtro de inferencias por Dispositivo', () => {
    ...
    test('El filtro de inferencias por dispositivo funciona', async ({ page }) => {
        ...
        await inferencePage.selectLocation('Oficina SIA');

        await inferencePage.selectGroup('GRUPO PANTALLAS LAB');
        ...
        inferences.forEach((inference: any, index: number) => {
            ...
            expect(inference.groupId, `Error en la inferencia #${index + 1} (ID: ${inference.id})`).toBe(1);
        });
