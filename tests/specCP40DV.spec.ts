...
test.describe('Filtro de inferencias por Dispositivo', () => {
    ...
    test('El filtro de inferencias por dispositivo funciona', async ({ page }) => {
        ...
        await inferencePage.selectDevice('GT RK3588 1');
        ...
        inferences.forEach((inference: any, index: number) => {
            ...
            expect(inference.boxId, `Error en la inferencia #${index + 1} (ID: ${inference.id})`).toBe(62);
        });
        console.log('✅ Validación exitosa: Todas las inferencias corresponden al boxId 62.');
    });
});